...
class RuGPT3XL(PreTrainedModel):
    def __init__(self, model, tokenizer, model_path, seq_len=512, min_generated_len=32):
        ...
        self.min_generated_len = min_generated_len
    ...
    def from_pretrained(
        ...
        master_port="6000",
        min_generated_len=32,
        rank=0
    ):
        init_method = 'tcp://' + os.getenv('MASTER_ADDR', 'localhost') + ':' + os.getenv('MASTER_PORT', master_port)
        try:
            torch.distributed.init_process_group(backend='nccl', world_size=1, rank=rank, init_method=init_method)
        ...
        # model.cuda()
        model = model.eval()
        return cls(model, tokenizer=tokenizer, seq_len=seq_len, model_path=model_name_or_path, min_generated_len=min_generated_len)
